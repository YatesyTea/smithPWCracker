...
void CopySolutionFromOpenListIntoWorkingCandidate(int solutionIndex)
{
   //define the global variables as having beeen declared in a another file
    ...
    if (solutionIndex==0 && openList.size>1 && openList.listEntries[solutionIndex].score<highestval)
        callfreqs[OLDEST]++;
    else if (solutionIndex== openList.size-1  &&  openList.size>1 && openList.listEntries[solutionIndex].score<highestval)
        callfreqs[YOUNGEST]++;
    else if (openList.listEntries[solutionIndex].score  ==lowestval && lowestval < highestval)
...
#ifdef ONDEWIS
    extern void markingChecks2(void);
    markingChecks2();
#endif
